...
.bpContainer{pointer-events:auto!important}`;
  ...
  /* v22.5.54 — the icon is now OUR element with an inline SVG, layered above
     whatever Botpress renders. Botpress's own webchat bundle (v3.6/inject.js)
     is a floating URL — different browsers hold different builds with different
...
.bpFabWrapper .bpUnreadMessage{display:none!important}`;
  ...
  /* v22.5.54 — our own FAB icon overlay (logo closed / chevron open) */
  function injectFabLogo(sh) {
    var fab = sh.querySelector('.bpFab');
    if (!fab) return;
    fab.style.setProperty('position', 'relative', 'important');
    var el = fab.querySelector('.achord-fab-logo');
    if (!el) {
      el = document.createElement('div');
      el.className = 'achord-fab-logo';
      el.setAttribute('aria-hidden', 'true');
      var s2 = el.style;
      s2.setProperty('position', 'absolute', 'important');
      s2.setProperty('inset', '0', 'important');
      s2.setProperty('display', 'flex', 'important');
      s2.setProperty('align-items', 'center', 'important');
      s2.setProperty('justify-content', 'center', 'important');
      s2.setProperty('pointer-events', 'none', 'important');
      s2.setProperty('z-index', '3', 'important');
      s2.setProperty('opacity', '1', 'important');
      fab.appendChild(el);
    }
    var open = fab.classList.contains('achord-fab-open');
    var want = open ? 'chev' : 'logo';
  ...
  function positionFab(sh) {
    ...
    var base = isMobile() ? 25 : 28;   /* mobile: the contact button's original
       height — the pair may slightly cover the footer / Framer badge, by design
       (v22.5.54, per Yoni) */
    var gap = 12;
    var host = document.getElementById('fab-root');
